...
// Pure-Swift SHA-256 fallback used wherever CryptoKit is unavailable (Linux).
// Extracted from MSPAgentModelRequestEvidence so the model catalog (which needs
// a credential-scope digest) can reuse the same implementation instead of
// pulling in CryptoKit — which is not available on Linux.
